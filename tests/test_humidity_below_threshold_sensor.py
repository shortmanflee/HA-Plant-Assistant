"""Tests for humidity below threshold weekly duration sensor."""
...
from custom_components.plant_assistant.sensor import (
    HumidityBelowThresholdHoursSensor,
)
...
async def test_humidity_below_threshold_sensor_init(mock_hass):
    """Test initialization of HumidityBelowThresholdHoursSensor."""
    sensor = HumidityBelowThresholdHoursSensor(
        hass=mock_hass,
    ...
    assert sensor._attr_name == "Test Garden Humidity Below Threshold Weekly Duration"
    expected = (
        f"{DOMAIN}_test_entry_123_test_garden_humidity_below_threshold_weekly_duration"
    )
...
async def test_humidity_below_threshold_sensor_available(mock_hass):
...
async def test_humidity_below_threshold_no_min_humidity_entity(mock_hass):
    """Test calculation when no minimum humidity entity exists."""
    sensor = HumidityBelowThresholdHoursSensor(
        hass=mock_hass,
        entry_id="test_entry",
        location_device_id="test_location",
    ...
    # No min humidity entity in registry
    result = await sensor._calculate_hours_below_threshold()
    assert result is None


async def test_humidity_below_threshold_min_humidity_unavailable(
    mock_hass, mock_entity_registry
):
    """Test calculation when minimum humidity threshold is unavailable."""
    # Set up mock entity registry with min_humidity entity
    mock_min_humidity_entity = MagicMock()
    mock_min_humidity_entity.platform = DOMAIN
    mock_min_humidity_entity.domain = "sensor"
    mock_min_humidity_entity.unique_id = f"{DOMAIN}_test_entry_min_humidity"
    mock_min_humidity_entity.entity_id = "sensor.test_garden_min_humidity"
    mock_entity_registry.entities.values.return_value = [mock_min_humidity_entity]
    ...
    # Mock min humidity state as unavailable
    mock_min_humidity_state = MagicMock()
    mock_min_humidity_state.state = STATE_UNAVAILABLE
    mock_hass.states.get.return_value = mock_min_humidity_state

    result = await sensor._calculate_hours_below_threshold()
    assert result is None


async def test_humidity_below_threshold_no_statistics(mock_hass, mock_entity_registry):
    ...
    # Mock min humidity state
    mock_min_humidity_state = MagicMock()
    mock_min_humidity_state.state = "45.0"
    mock_hass.states.get.return_value = mock_min_humidity_state
...
async def test_humidity_below_threshold_calculation(mock_hass, mock_entity_registry):
    ...
    # Mock min humidity state (threshold is 50%)
    mock_min_humidity_state = MagicMock()
    mock_min_humidity_state.state = "50.0"
    mock_hass.states.get.return_value = mock_min_humidity_state

    # Mock statistics data
    mock_stats = {
        "sensor.test_humidity": [
            {"mean": 45.0},  # Below threshold
            {"mean": 48.5},  # Below threshold
            {"mean": 52.0},  # Above threshold
            {"mean": 42.0},  # Below threshold
            {"mean": 60.0},  # Above threshold
        ]
    }
    ...
    with (
        ...
        result = await sensor._calculate_hours_below_threshold()
        # Should count 3 hours below threshold (45.0, 48.5, 42.0)
        assert result == 3
